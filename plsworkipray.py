...
class Config:
    ...
    target_name: str
    background_fasta: Optional[str]  # optional now
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Config + crRNA + primers + pairing + features + optional BLAST background (per-component)"
    )
    # Required
...
def annotate_on_target_uniqueness(ps: PairedSet, target: str, cfg: Config):
    ...
    # RP binding site on + strand (top) = revcomp(primer seq)
    rp_site_top = revcomp(ps.rp.seq)

    # exact full matches anywhere on target (both strands for primers)
    ...
    # exclude the designed exact site from mismatch-tolerant totals (since mm=0 included)
    ...
    # On-target valid amplicons (exact FP site + exact RP site that yield valid length)
    min_amp, max_amp = cfg.amplicon_len_range
...
# =========================
# BLAST (mismatch-aware) — PER-COMPONENT ONLY (optional background)
# =========================
...
def write_pairs_csv(pairs: List[PairedSet], out_path: str, target_name: str, background_used: bool):
    fields = [
        ...
        "background_used",
        # mismatch-aware individual off-targets vs BACKGROUND (best-any + best-imperfect) + exact flags
    ...
    with open(out_path, "w", newline="") as f:
        ...
        for ps in pairs:
            ...
            row = {
                ...
                # on-target uniqueness & approximate counts
                ...
                # defaults for BACKGROUND-based BLAST metrics (neutral if no background)
...
if __name__ == "__main__":
    try:
        ...
        for ps in pairs:
            ...
            annotate_on_target_uniqueness(ps, tseq, cfg)  # includes target-only approximate counts
        ...
        for ps in pairs[:show_n]:
            ...
            print(
                ...
                f"{'PAM='+g.pam_seq if g.pam_seq else 'PAM=NA'} prot@{g.protospacer_start} "
                f"len={len(g.protospacer_seq)} GC={g.gc_pct:.1f}%"
            )
            print(
            ...
            print(
                f"  Seed seq={ps.metrics['seed_seq']}  GC={ps.metrics['seed_gc_pct']:.1f}%  "
                f"max_run={int(ps.metrics['seed_max_run'])}"
            )
            print(
                f"  On-target approx: FP mm1_excl={int(ps.metrics['fp_full_hits_mm1_excl'])}, "
                f"RP mm1_excl={int(ps.metrics['rp_full_hits_mm1_excl'])}, "
                f"g mm1_excl={int(ps.metrics['crrna_full_hits_mm1_excl'])}"
            )

            # Per-component off-target summary vs BACKGROUND (if any)
        ...
        # CSV
        write_pairs_csv(pairs, out_csv, tname, background_used)
        print(
            "\n[OK] Features complete. "
            + (
                "Included BLAST specificity vs background."
                if background_used
                else "No background provided; skipped BLAST and wrote target-only mismatch counts."
            )
        )
